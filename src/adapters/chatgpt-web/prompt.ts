...
export function compileChatGptWebPrompt(
  ...
  const sharedContract = [
    ...
    "Treat filesystem paths according to their provenance. Paths originating from the supplied outer Codex task, its environment context, user file references, or Codex Native tool results belong to the outer Codex filesystem unless their source explicitly identifies a different environment.",
    "External apps, remote MCP servers, cloud development environments, remote workspaces, containers, and other tool backends have separate filesystem namespaces. Never assume that a path visible in one environment exists in another, and never use a remote or external tool to resolve a path owned by the outer Codex filesystem.",
    "When data must cross an environment boundary, first read it from the source environment, explicitly transfer only the required data, and then use only a destination path confirmed by the destination environment. Determine path ownership from provenance rather than path syntax; if ownership is ambiguous, establish the originating environment before attempting filesystem access.",
